...
The `update-tree.yml` workflow posts roots on-chain automatically via `leanimt-plus-build --post-root` after building LeanIMT+ snapshots. It reads `root.json` files and calls `LeanIMTPlusRootStorage.setRoot()` for each issuer. Skips gracefully when secrets are not configured (forks/PRs).
